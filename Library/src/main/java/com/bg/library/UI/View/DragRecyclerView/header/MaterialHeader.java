...
import com.bg.library.UI.View.DragRecyclerView.DragUIHandlerHook;
import com.bg.library.UI.View.DragRecyclerView.indicator.DragIndicator;

public class MaterialHeader extends View implements DragUIHandler {
    ...
    public void setDragFrameLayout(DragFrameLayout layout) {

        final DragUIHandlerHook mDragUIHandlerHook = new DragUIHandlerHook() {
            @Override
            public void run() {
        ...
        mDragFrameLayout = layout;
        mDragFrameLayout.setRefreshCompleteHook(mDragUIHandlerHook);
    }
    ...
    public void onUIPositionChange(DragFrameLayout frame, boolean isUnderTouch, byte status, DragIndicator dragIndicator) {
        ...
        float percent = Math.min(1f, dragIndicator.getCurrentPercent());

        if (status == DragFrameLayout.DRAG_STATUS_PREPARE) {
